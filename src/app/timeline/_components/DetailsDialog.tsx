				...
				<div className="mt-4 min-h-0 flex-1 overflow-y-auto pr-1">
					...
					{n && (
						<div className="space-y-4">
							<div>
								{n.theme && (
									<div className="mt-1 inline-flex rounded-full bg-emerald-50 px-2 py-0.5 text-emerald-700 text-xs dark:bg-emerald-950/40 dark:text-emerald-200">
										{n.theme}
									</div>
								)}
								{n.source && (
								...
								{n.era && (
									<div className="mt-1 text-xs text-zinc-500">era: {n.era}</div>
								)}
								{n.year != null && (
									<div className="mt-1 text-xs text-zinc-500">
										year: {n.year}
								...
								{n.tags && n.tags.length > 0 && (
									<div className="mt-2 flex flex-wrap gap-1">
										{n.tags.slice(0, 16).map((t) => (
											<span
												key={t}
												className="rounded-full bg-purple-50 px-2 py-0.5 text-purple-700 text-xs dark:bg-purple-950/40 dark:text-purple-200"
											>
												{t}
											</span>
										))}
								...
								{n.people && n.people.length > 0 && (
									<div className="mt-2 flex flex-wrap gap-1">
										{n.people.slice(0, 16).map((p) => (
											<span
												key={p}
												className="rounded-full bg-orange-50 px-2 py-0.5 text-orange-700 text-xs dark:bg-orange-950/40 dark:text-orange-200"
											>
												{p}
											</span>
										))}
							...
							{nodeType(n) === "event" && (
								<>
									<div className="markdown rounded-lg border border-zinc-200 bg-zinc-50 p-3 text-sm text-zinc-700 leading-6 dark:border-zinc-800 dark:bg-zinc-900/30 dark:text-zinc-200">
										<ReactMarkdown
							...
							<div>
								<div className="font-medium text-xs text-zinc-600 dark:text-zinc-300">
									Neighbors
								</div>
								<div className="mt-2 flex flex-wrap gap-2">
									{[...(props.adjacency.get(n.id) ?? [])].map((id) => {
